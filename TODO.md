...
- Make `scripts/setup-third-party.sh` leave a buildable `third_party/` tree; fresh checkouts still fail `./gradlew assembleDebug` because `app/src/main/cpp/CMakeLists.txt` `add_subdirectory()` points at a missing directory.
