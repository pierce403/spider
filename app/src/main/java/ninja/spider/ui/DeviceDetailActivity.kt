...
import ninja.spider.data.DeviceEntity
import ninja.spider.databinding.ActivityDeviceDetailBinding
...
class DeviceDetailActivity : AppCompatActivity() {
      ...
      repeatOnLifecycle(Lifecycle.State.STARTED) {
        launch {
          repository.observeDevice(deviceKey).collect { device ->
            ...
            binding.detailKey.text = getString(ninja.spider.R.string.sensor_key_value, device.deviceKey)
            binding.detailFirstSeen.text = getString(
              ninja.spider.R.string.first_seen_value,
              Formatters.formatTimestamp(device.firstSeen)
            )
            binding.detailLastSeen.text = getString(
              ninja.spider.R.string.last_seen_value,
              Formatters.formatTimestamp(device.lastSeen)
            )
            binding.detailStats.text = getString(
              ninja.spider.R.string.detail_stats_value,
              device.rssiMin,
              device.rssiMax,
              device.rssiAvg,
            ...
            binding.detailMetadata.text = metadata.rawJson ?: device.lastMetadataJson ?: getString(ninja.spider.R.string.no_metadata_recorded)
          }
        }
  ...
  override fun onCreateOptionsMenu(menu: Menu): Boolean {
    menuInflater.inflate(ninja.spider.R.menu.device_detail_menu, menu)
    return true
  }

  override fun onPrepareOptionsMenu(menu: Menu): Boolean {
    val exportReady = currentDevice != null
    menu.findItem(ninja.spider.R.id.menu_rename_device)?.isEnabled = exportReady
    menu.findItem(ninja.spider.R.id.menu_copy_device_json)?.isEnabled = exportReady
    menu.findItem(ninja.spider.R.id.menu_save_device_json)?.isEnabled = exportReady
    menu.findItem(ninja.spider.R.id.menu_share_device_json)?.isEnabled = exportReady
    return super.onPrepareOptionsMenu(menu)
  }

  override fun onOptionsItemSelected(item: MenuItem): Boolean {
    return when (item.itemId) {
      ninja.spider.R.id.menu_rename_device -> {
        showRenameDialog()
        true
      }
      ninja.spider.R.id.menu_copy_device_json -> {
        copyCurrentDeviceJson()
        true
      }
      ninja.spider.R.id.menu_save_device_json -> {
        saveCurrentDeviceJson()
        true
      }
      ninja.spider.R.id.menu_share_device_json -> {
  ...
  private fun showRenameDialog() {
    ...
    val input = android.widget.EditText(this).apply {
      ...
      hint = getString(ninja.spider.R.string.rename_device_hint)
      selectAll()
    }
    ...
    com.google.android.material.dialog.MaterialAlertDialogBuilder(this)
      .setTitle(ninja.spider.R.string.rename_device_title)
      .setView(container)
      .setMessage(ninja.spider.R.string.rename_device_clear_hint)
  ...
  private fun buildLatestReading(metadata: ninja.spider.util.TpmsMetadata): String {
    return buildList {
      ...
      getString(ninja.spider.R.string.no_latest_reading)
    }
  }
  ...
  private fun copyCurrentDeviceJson() {
    ...
    getSystemService(ClipboardManager::class.java)?.setPrimaryClip(
      ClipData.newPlainText(getString(ninja.spider.R.string.copy_device_json), export.json)
    )
    Toast.makeText(
      this,
      ninja.spider.R.string.device_json_copied,
      Toast.LENGTH_SHORT
    ).show()
  }
      ...
      lifecycleScope.launch {
        ...
        if (saved) {
          Toast.makeText(
            ...
            getString(ninja.spider.R.string.device_json_saved_downloads, export.fileName),
            Toast.LENGTH_SHORT
          ).show()
        } else {
          Toast.makeText(
            this@DeviceDetailActivity,
            ninja.spider.R.string.device_json_save_failed,
            Toast.LENGTH_SHORT
          ).show()
        }
      }
  ...
  private fun shareCurrentDeviceJson() {
    ...
    val shareIntent = Intent(Intent.ACTION_SEND).apply {
      type = "text/plain"
      putExtra(Intent.EXTRA_SUBJECT, getString(ninja.spider.R.string.device_json_share_subject, export.fileName))
      putExtra(Intent.EXTRA_TEXT, export.json)
    }
    startActivity(Intent.createChooser(shareIntent, getString(ninja.spider.R.string.share_device_json)))
  }
  ...
  private fun writePendingExportToUri(uri: Uri) {
    ...
    lifecycleScope.launch {
      ...
      Toast.makeText(
        this@DeviceDetailActivity,
        if (saved) {
          getString(ninja.spider.R.string.device_json_saved, export.fileName)
        } else {
          getString(ninja.spider.R.string.device_json_save_failed)
        },
        Toast.LENGTH_SHORT
      ).show()
    }
